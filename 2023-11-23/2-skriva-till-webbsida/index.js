...
let mysql = require("mysql2"); // installera med kommandot "npm install" som vanligt
  ...
  con.connect(function (err) {
    ...
    con.query("SELECT * FROM users", function (err, result, fields) {
      ...
      fs.readFile("mall.html", "utf-8", function (err, data) {
        ...
        console.log("First####"+ htmlArray[0])
        for (let key in result[0]) {
          output += `<th>${key}</th>`; // print ut Ämne
        }
        output += htmlArray[1];  //htmlArray[1] is  </tr>
        console.log("Second####"+ htmlArray[1])
        for (let user of result) {
          output += "<tr>";          // Starta med tr ämne
          for (let key in user) {
            output += `<td>${user[key]}</td>`; // print ut users
          }
          output += "</tr>";
        }
        output += htmlArray[2];
        console.log("Third####"+ htmlArray[2])
        res.send(output);
      });
    });
  });
